...
explicitly provided (e.g., patient date of birth, policy inception date,
...
IF EITHER prescriptionValid OR policyValid is false:
   ...
   - Set overallStatus based on which is invalid:
     - If both are false: "Invalid Policy and Prescription - Cannot Process"
     - If only policyValid is false: "Invalid Policy - Cannot Process"
     - If only prescriptionValid is false: "Invalid Prescription - Cannot Process"
   - Set overallConfidence = 0
   - Populate documentValidity fully.
...
b. POLICY PERIOD VALIDITY (policyEligibility.periodEligible):
   DATA SOURCES (strictly enforced):
     - Policy period start/end dates: sourced ONLY from the POLICY
       document text. If the policy document does not explicitly state
       a coverage period or expiry date, these are considered absent.
     - Medical event/consult date: sourced ONLY from the PRESCRIPTION
       document (e.g., "Consult Date", "Visit Date", "Date of Service").
     - Any date embedded in the prescription's "Policy Info" or
       "Exp:" annotations is NOT a policy document date — ignore it
       completely for period validation purposes.

   DETERMINATION:
   - If the policy document explicitly contains a coverage period and
     the prescription's consult/visit date falls OUTSIDE that window
     and both values are clearly present and conflict → periodEligible = false.
   - If the policy document does NOT contain explicit period/expiry
     dates → set periodEligible = true, note in explanation that policy
     period dates were not found in the policy document, and proceed
     with item-level analysis normally.
   - Only a CONFIRMED mismatch using policy-document-sourced dates
     justifies setting periodEligible = false.
...
IF ANY of ageEligible, periodEligible, or classEligible is false:
   ...
   - For EVERY item in comparison[], set:
       ...
       - confidence = 95 (or overallConfidence)
...
`;
  ...
  try {
    ...
    let rawComparison = parsedResponse.comparison || [];

    // Fallback: If policy eligibility failed and LLM returned an empty comparison array,
    // automatically populate comparison items from prescriptionJson marked as Not Covered.
    if (rawComparison.length === 0 && prescriptionJson) {
      const eligibilityExp = (parsedResponse.policyEligibility && parsedResponse.policyEligibility.explanation) || parsedResponse.summary || 'Policy eligibility failed.';
      const fallbackItems = [];

      if (Array.isArray(prescriptionJson.medicalTests) && prescriptionJson.medicalTests.length > 0) {
        prescriptionJson.medicalTests.forEach(t => {
          if (t && (t.name || t)) {
            fallbackItems.push({
              item: typeof t === 'string' ? t : t.name,
              itemType: 'Medical Test',
              prescriptionCost: (typeof t === 'object' && t.cost) ? t.cost : 0,
              coverageStatus: 'Not Covered',
              coverageStatusReason: `Not Covered: ${eligibilityExp}`,
              explanation: `Not Covered: ${eligibilityExp}`,
              policyEvidence: '',
              confidence: parseConfidence(parsedResponse.overallConfidence) || 95,
              financialDecision: 'Not Payable due to Policy Eligibility Failure'
            });
          }
        });
      ...
      if (Array.isArray(prescriptionJson.procedures) && prescriptionJson.procedures.length > 0) {
        prescriptionJson.procedures.forEach(p => {
          if (p && (p.name || p)) {
            fallbackItems.push({
              item: typeof p === 'string' ? p : p.name,
              itemType: 'Procedure',
              prescriptionCost: (typeof p === 'object' && p.cost) ? p.cost : 0,
              coverageStatus: 'Not Covered',
              coverageStatusReason: `Not Covered: ${eligibilityExp}`,
              explanation: `Not Covered: ${eligibilityExp}`,
              policyEvidence: '',
    ...
    const enhancedResponse = {
      documentValidity: parsedResponse.documentValidity || {
        prescriptionValid: true,
        policyValid: true,
        injectionAttemptDetected: false,
        injectionAttemptDetails: '',
      ...
      comparison: filteredComparison.map(c => ({
        item: c.item || '',
        itemType: c.itemType || '',
        prescriptionCost: typeof c.prescriptionCost === 'number' ? c.prescriptionCost : (c.cost || 0),
        cost: typeof c.prescriptionCost === 'number' ? c.prescriptionCost : (c.cost || 0),
        coverageStatus: c.coverageStatus || c.status || '',
        status: c.coverageStatus || c.status || '',
        coverageStatusReason: c.coverageStatusReason || '',
        policyLimit: c.policyLimit !== undefined ? c.policyLimit : null,
        estimatedPayableAmount: c.estimatedPayableAmount !== undefined ? c.estimatedPayableAmount : null,
        estimatedPatientPayable: c.estimatedPatientPayable !== undefined ? c.estimatedPatientPayable : null,
        coPayment: c.coPayment !== undefined ? c.coPayment : null,
        deductible: c.deductible !== undefined ? c.deductible : null,
        waitingPeriodApplicable: Boolean(c.waitingPeriodApplicable),
        waitingPeriodVerified: Boolean(c.waitingPeriodVerified),
        networkHospitalRequired: Boolean(c.networkHospitalRequired),
        cashlessEligible: Boolean(c.cashlessEligible),
